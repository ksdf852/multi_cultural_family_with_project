...
void loop() {
  ...
  if(buttonState==LOW && count<50){
 // colorWipe(strip.Color(colors[i%6][0], colors[i%6][1], colors[i%6][2]), 1);
 // i++;
   digitalWrite(ddd1Pin,HIGH);
   digitalWrite(lazerPin,HIGH);  
   delay(30);  
   digitalWrite(lazerPin,LOW);   
   root.rewind();
   play(root);
   digitalWrite(ddd1Pin,LOW);
